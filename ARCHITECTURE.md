...
2. **The LLM explains, it never invents.** It receives structured findings and engine-derived move lists. It may name moves only through the structured `mentionedMoves` field, each checked by exact match against that engine data; free-text fields must contain no move-like content and no piece names at all (squares in any disguise, castling, promotion, move numbers, piece words, after Unicode normalisation). Any violation causes a fallback to the templated text, which is generated by our own code from engine data. Residual risk: pure prose that names a move with no notation and no piece word cannot be excluded by any rule; the prompt forbids it (see `docs/KNOWN-LIMITATIONS.md`).
