...
- LLM move validation: moves are accepted only through `mentionedMoves` (exact match with engine data), and free text is rejected if it contains move-like content after normalisation (NFKD, invisible characters removed, Cyrillic/Greek look-alikes folded, non-Latin letters and non-ASCII digits rejected). Residual risk that no regex can close: pure prose that names a move with no square or notation (for example a paraphrase we did not anticipate). The prompt forbids it and common phrasings (English, plus German/French/Spanish/Russian-transliterated piece words) are caught, but the non-English piece-word lists were not reviewed by native speakers. Also not handled: visual digit look-alikes such as "l" or "I" for 1, and spelled-out squares in other languages. The template fallback text names engine moves inline by design and is not subject to this rule.
